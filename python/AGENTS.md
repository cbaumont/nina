...
* Stack: Python 3.13, `claude-agent-sdk`, Textual TUI, `watchdog` for file-idle detection, git plumbing for workspace snapshots.
* Layout: `src/nina/cli.py` (entry point), `src/nina/engine.py` (step state machine, typing-dial policy, tool handlers), `src/nina/prompts.py` (system + turn prompts), `src/nina/screening.py` (haiku leak classifier), `src/nina/tools.py` (tool specs), `src/nina/workspace.py` (git snapshots/diffs under `refs/nina/*`), `src/nina/agent/` (the `AgentSession` protocol and its SDK implementation), `src/nina/tui/` (Textual UI).
* The typing dial is enforced inside tool handler bodies in `engine.py` (never in the prompt alone) — keep it that way. Same for the leak-screening rewrite gate (`Engine.rewriting`).
